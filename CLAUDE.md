...
- **`/admin` is the app's one cross-user read, and its own security boundary is the API route, not the page.** `isAdminEmail()` (`src/lib/admin/access.ts`) checks a verified ID token's `email` claim against the server-only `ADMIN_EMAIL` env var — fails closed if unset (tested). `GET /api/admin/dashboard` and `GET /api/admin/users/[uid]` are the only routes that read across all users/challenges via the Admin SDK; both re-check `isAdminEmail` themselves rather than trusting the page. **No `firestore.rules` change was made or needed** — deliberately avoided by keeping the fan-out server-side. The `/admin` and `/admin/users/[uid]` pages have no server-side gate of their own (any signed-in user can load the page shell; the API 403s and no data ever reaches a non-admin) — consistent with "client-side code isn't trusted" elsewhere in this file. No nav link anywhere — Andreas reaches it by URL only, on purpose, so it never renders for anyone else. The KPI math itself (`buildAdminDashboard` in `src/lib/admin/stats.ts`) is pure and Admin-SDK-free, so it's unit-tested directly rather than through mocked Firestore calls.
...
- [x] ~~`ADMIN_EMAIL` not set~~ — **fixed**, set in both `.env.local` and Netlify; `/admin` confirmed working.
- [x] ~~PWA icons are placeholder art~~ — **fixed**, real logo artwork (white/light-purple double-chevron mark on the purple rounded-square background) now in `src/app/icon.png`, `apple-icon.png`, `public/icons/icon-{192,512}.png`.
