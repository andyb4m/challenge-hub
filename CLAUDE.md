...
- [ ] **`ADMIN_EMAIL` not set.** Without it, `/admin` 403s for everyone, including Andreas — `isAdminEmail()` fails closed on an unset value by design. Set it to the exact email his Challenge Hub account signs in with (case-insensitive), in both `.env.local` and Netlify.
