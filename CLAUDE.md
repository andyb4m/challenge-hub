...
- [ ] **Strava's "Authorization Callback Domain" (strava.com/settings/api) doesn't match the live domain — confirmed actively broken for anyone but Andreas.** Connecting Strava fails with `{"errors":[{"field":"redirect_uri","code":"invalid"}]}` for every user except Andreas, because Strava always whitelists `localhost` regardless of the configured domain, and his own testing happened there — masking the bug. Needs the Strava dashboard setting updated to `fit-challenge-hub.netlify.app` (bare domain, no scheme/path). Pure account-settings fix, not code; only Andreas can do it. Revisit again if/when a custom domain replaces the Netlify one.
